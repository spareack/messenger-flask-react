    ...
    return (
        <div className={classes.companion} style={companion?.other_members.length ? {display: 'flex'} : {display: 'none'} }>
            <div className={classes.companionInfo}>
                ...
                <div><h2 style={{marginLeft: companion?.other_members?.length ? '' : '15px'}}>{companion?.other_members?.length? companion?.other_members.length === 1? companion?.other_members :'Название группового разговора': 'Выберите диалог'}</h2>
                <p>{companion?.other_members?.length ? 'last seen recently' : ''}</p></div>
            </div>
