...
import {sendMessage, getMessages, createTalk} from '../libs/chatAPI'
import { isCurrentDay, isYesterday, Separator, Talk } from '../libs/constructor'
...
const MainWindow = ({companion, active, setActiveTalkMenu}) => {
    ...
    const sendMessageLocal = async (e) => {
        ...
        if(messageText !== '')
        {
            if(isCurrentDay(talks.filter(item => item.id === lastTalk)[0].date)){
                sendMessage(messageText, user.id, lastTalk, currentDialog)
                console.log(isCurrentDay(talks.filter(item => item.id === lastTalk)[0].date), talks.filter(item => item.id === lastTalk)[0].date)
            }
            else {
                ...
                dispatch({type: 'setMessages', payload: [new Separator(isYesterday('123', true)), ...messages]})
